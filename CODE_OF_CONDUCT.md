...
repository — issues, pull requests, discussions, and any other channel
used to interact around this project — and also applies when an
individual is officially representing the project in public spaces.

## Enforcement

This project has a single maintainer rather than a dedicated moderation
team, so reports are handled directly by that maintainer. Instances of
abusive, harassing, or otherwise unacceptable behavior may be reported
through either of these channels:

* A **private security advisory** on this repository (GitHub → the
  **Security** tab → **Report a vulnerability**), which is visible only
  to the maintainer and the reporter — the same private channel used for
  security reports, described in [`SECURITY.md`](SECURITY.md).
* A regular **GitHub issue**, for anything that doesn't need to stay
  confidential.
