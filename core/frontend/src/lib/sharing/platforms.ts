 ...
 *
 * Defines available sharing platforms and their URL templates.
...
export interface SharePlatform {
  ...
  name: string
  displayName: string
  /** Brand color for the platform icon */
  color: string
  /** SVG path for the platform icon */
  iconPath: string
  /** Generate the share URL for this platform */
...
const DEFAULT_SHARE_TEXT = 'Made with Sterna'

/**
 * Twitter/X sharing
 */
const twitter: SharePlatform = {
  id: 'twitter',
  name: 'twitter',
  displayName: 'X',
  color: '#000000',
  ...
  getShareUrl: ({ url, text = DEFAULT_SHARE_TEXT }) =>
...
const reddit: SharePlatform = {
  ...
  getShareUrl: ({ url, title = DEFAULT_SHARE_TEXT }) =>
    `https://reddit.com/submit?url=${encodeURIComponent(url)}&title=${encodeURIComponent(title)}`,
}

/**
 * All available sharing platforms
 */
