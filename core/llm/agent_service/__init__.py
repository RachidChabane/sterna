...
into a running turn lives here instead -- which tools it may call,
which key it streams on, how its frames are rendered, and how it is
billed.

Both chat endpoints run their turns on that loop and each states its
own terms. A V2 request describes its turn with `TurnRequest` and runs
it with `V2TurnRunner`; a direct-completion request is answered whole
by `v1_endpoint.v1_streaming_response`.
"""
